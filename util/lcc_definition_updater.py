...
class LCCDataFetcher:
    ...
    def find_by_tp2(self, tp2_name: str) -> Optional[dict[str, Any]]:
        ...
        return self.clean_mod_data(mod_data)

    def clean_mod_data(self, mod_data: dict) -> dict[str, Any]:
        """
        Transforme les données d'un mod LCC en données utilisables par le BWS
        """
        ...
        for lang, lang_code in self.LANGUAGE_CODES.items():
            ...
            if translated_mod_data:
                if translated_mod_data.get("description"):
                    description = translated_mod_data["description"]
                else:
                    # fallback
                    description = self.id_index[str(mod_data["id"])].get("description", "")
...
class JSONCompleter:
    ...
    def complete_file(self, json_path: Path) -> bool:
        ...
        try:
            ...
            # Déterminer le tp2 depuis le nom du fichier, format `{tp2}-{id}`
            tp2_name, _, mod_id = json_path.stem.rpartition("-")

            self.log(f"Traitement: {json_path.name}")

            # Chercher dans LCC
            lcc_data = self.fetcher._find_mod_by_id(mod_id, lang="default")

            if not lcc_data:
                self.log(f"  ⚠ Mod non trouvé dans LCC: {mod_id} ({tp2_name})", "WARNING")
                self.stats["not_found"] += 1
                self.not_found_list.append(tp2_name)
                return False

            lcc_data = self.fetcher.clean_mod_data(lcc_data)
            # Appliquer les données
            updated = False

            # ID
            if not mod_data.get("id"):
                mod_data["id"] = lcc_data["id"]
                updated = True
                self.log(f"  + id: {lcc_data['id']}")
            ...
            # Descriptions
            if "translations" not in mod_data:
                mod_data["translations"] = dict()
